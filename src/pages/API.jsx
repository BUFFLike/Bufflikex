...
export default function API() {
  ...
  const generateKey = async () => {
    ...
    try {
      ...
      setApiKey(response.data.data?.apiKey || response.data.apiKey || '');
      showToast('Da tao API key moi');
    } catch (err) {
      showToast('Tao API key that bai');
    } finally {
      setGenerating(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-white mb-1">API Access</h1>
        <p className="text-sm text-[#a0a0b0]">Tich hop webbuffMXH voi ung dung cua ban</p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-[#1a1a2e] border border-[#2d2d44] rounded-xl">
            <div className="flex items-center gap-2 px-5 py-4 border-b border-[#2d2d44]">
              ...
              <h3 className="font-semibold text-white">API Key cua ban</h3>
            </div>
            <div className="p-5">
